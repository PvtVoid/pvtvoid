# Hi, I'm PvtVoid! 👋

![Typing SVG](https://readme-typing-svg.herokuapp.com?font=Fira+Code&pause=1000&color=F7B731&width=435&lines=Lead+Developer+of+Talishar;Game+Logic+%7C+Full-Stack+Development;Community-Driven+Fan+Project)

I'm a **creative**, **passionate**, and **goal-oriented** developer dedicated to making Talishar as good as possible. With a focus on game logic, backend architecture, and community engagement, I'm committed to creating an accessible platform for players worldwide.

---

## 💼 About Me

- **Lead Developer** of [Talishar](https://talishar.net/) - A browser-based Flesh and Blood TCG platform (Core contributor & architect)
- Passionate about games, logic, and this community
- Full-stack developer skilled in backend and frontend technologies
- Community-driven advocate believing in open-source collaboration
- Currently building the most ambicious online Flesh and Blood experience

---

## 🔥 What I'm Working On

### Talishar Platform
**The complete browser-based Flesh and Blood TCG experience**

- **[Talishar](https://github.com/Talishar/Talishar)** - PHP backend with game logic engine
  - Core game mechanics and rule implementation
  - Card ability system with 1000+ cards
  - Game state management and persistence

- **[Talishar-FE](https://github.com/Talishar/Talishar-FE)** - TypeScript/React frontend
  - Real-time game board interface
  - Responsive design for desktop and mobile
  - Seamless player experience

---

### 👨‍💻 Languages & Specializations
- **Backend**: PHP, C#
- **Frontend**: React, CSS
- **Game Logic**: Card mechanics, state management, turn-based systems

---

## 📊 Talishar Community Stats

[![Discord](https://flat.badgen.net/discord/online-members/JykuRkdd5S?icon=discord)](https://discord.gg/JykuRkdd5S)
[![GitHub Stars](https://img.shields.io/github/stars/Talishar/Talishar?style=flat-square&logo=github)](https://github.com/Talishar/Talishar)
[![GitHub Forks](https://img.shields.io/github/forks/Talishar/Talishar?style=flat-square&logo=github)](https://github.com/Talishar/Talishar)

---

## 🔥 GitHub Streak Stats

[![GitHub Streak](https://github-readme-stats.vercel.app/api?username=PvtVoid&type=streak&theme=dark)](https://github.com/anuraghazra/github-readme-stats)

### Key Achievements
- ✅ 1000+ card implementations
- ✅ Active community with 9000+ daily visitor
- ✅ Comprehensive game state management

---


## 🌟 Core Repositories

| Repository | Language | Purpose |
|------------|----------|---------|
| [Talishar](https://github.com/Talishar/Talishar) | PHP | Game engine & backend API |
| [Talishar-FE](https://github.com/Talishar/Talishar-FE) | TypeScript/React | Web interface & UI |
| [CardImages](https://github.com/Talishar/CardImages) | JavaScript | Card image processing |

---

## 🙋 Let's Connect

- **Website**: [Talishar.net](https://talishar.net/)
- **Discord**: [Talishar Community](https://discord.gg/JykuRkdd5S)
- **Twitter/X**: [@PvtVoid](https://twitter.com/PvtVoid)
- **Bluesky**: [@PvtVoid](https://bsky.app/profile/pvtvoid.bsky.social)
- **Patreon**: [PvtVoid](https://www.patreon.com/pvtvoid)
- **LinkedIn**: Available upon request

---

## 🎮 Support Talishar

Love playing Flesh and Blood on Talishar?
- **Become a Patron**: [Patreon](https://linktr.ee/Talishar)
- **Report Issues**: [GitHub Issues](https://github.com/Talishar/Talishar/issues)
- **Join Discord**: [Contribute & Community Server](https://discord.gg/JykuRkdd5S)

---

## Disclaimer

Talishar is a fan-made project. All artwork and card images © Legend Story Studios. Talishar.net is in no way affiliated with Legend Story Studios. Legend Story Studios®, Flesh and Blood™, and set names are trademarks of Legend Story Studios.

---

*Last updated: November 2025*

**Thank you for being part of the Talishar journey!** 🎮✨
